...
DistributionExperiment::DistributionExperiment(
    ...
    size_t sample_size) : dist_(std::move(dist)), sample_size_(sample_size)
{}
...
ExperimentStats DistributionExperiment::Run(std::mt19937& rng) {
    ...
    stats.empirical_mean = std::accumulate(
        run_results.begin(), 
        run_results.end(), 
        0.) / (double)sample_size_;

    auto tmp = run_results | std::views::transform (
        [&](double x) { return std::pow(x - stats.empirical_mean, 2); } 
    );
    
    stats.empirical_variance = std::accumulate(
        tmp.begin(),
        tmp.end(),
        0.
    ) / double(sample_size_ - 1);
...
double DistributionExperiment::KolmogorovDistance(
    const std::vector<double>& grid, 
    const std::vector<double>& empirical_cdf
) const {
    ...
    auto distances = std::views::iota(0u, grid.size()) | 
        std::views::transform(
            [&](int x) { return std::abs(theoretical_cdf[x] - empirical_cdf[x]);}
        );
